...
describe('AsignaturaProfesorPage', () => {
  let componente: AsignaturaProfesorPage;
  let fixture: ComponentFixture<AsignaturaProfesorPage>;
  let authServiceMock: any;

  // Datos simulados
  const mockProfesorId = '1'; // Profesor Carlos López
  ...
  const mockSubjectDetails = [
    {
      ...
      secciones: [
        ...
        },
        {
          ...
          profesor_id: '1', // Profesor Carlos López
          alumnos_inscritos: ['1'],
        },
        {
  ...
  describe('ngOnInit', () => {
    it('debería obtener y procesar correctamente las asignaturas', async () => {
      ...
      authServiceMock.getTeacherSubjects.and.returnValue(of(mockSubjectCodes).toPromise());
      ...
      expect(authServiceMock.getTeacherSubjects).toHaveBeenCalledWith('1');
    ...
    it('debería manejar el caso cuando no se encuentran asignaturas', async () => {
      authServiceMock.getTeacherSubjects.and.returnValue(of([]).toPromise());
      spyOn(console, 'warn');
    ...
    it('debería manejar los errores correctamente', async () => {
      authServiceMock.getTeacherSubjects.and.returnValue(Promise.reject('Error de red'));
      spyOn(console, 'error');
